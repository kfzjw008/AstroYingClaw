...
感谢@卷饼睡不醒 的硬件支撑相关工作，感谢 [@Chockmah](https://github.com/Mixsrter)  对本产品的鼎力协助，感谢[@MajorBill](https://github.com/MajorBill) 对本产品的技术大力指导。感谢[@东北林业大学天文爱好者协会](https://baike.baidu.com/item/%E4%B8%9C%E5%8C%97%E6%9E%97%E4%B8%9A%E5%A4%A7%E5%AD%A6%E5%A4%A9%E6%96%87%E7%88%B1%E5%A5%BD%E8%80%85%E5%8D%8F%E4%BC%9A/19145133) 的友情支持。
...
本项目（AstroYingClaw）作为软硬件结合的天文教学自动化插件，在系统架构与交互设计中，将安全性、透明度与社会价值放在首位，具体落实如下：

* **🔒 数据安全与隐私保护：** 本项目专为天文科普设计，核心数据流为天体坐标（经纬度、方位角/高度角）与教学文本。系统不收集、不存储、不上传任何用户的个人隐私数据。在硬件控制层面，通过本地配置的串口通信（Serial Port）与指星仪交互，实现执行环境的权限隔离，确保云端大模型无法直接越权读取本地设备的其他非相关数据。

* **👁️ 透明可控的“白盒”执行机制（核心安全设计）：**
  鉴于指星仪涉及激光物理指向，采用 **“AI生成课件 -> 结构化YAML -> 人类可审核 -> 自动化执行”** 的透明工作流。
  * **行为边界：** 智能体不会直接生成无约束的硬件动作，而是生成标准化的 YAML 流水线文件。用户在执行前可通过 `astro validate` 查阅所有即将执行的硬件指令。
  * **物理安全防护：** 平台提供 `astro stop` 紧急停止指令，随时可中断流水线。同时，在硬件原型机设计预期中，设备端设有俯仰角安全限制（例如禁止指向地平线以下或人群高度），确保激光指向的绝对安全。

* **⚖️ 合规与绿色使用：** 本插件生成的教学内容及语音朗读（TTS）严格限定在天文科学、地理知识及历史神话等科普范畴，依托 Python `ephem` 库进行天文计算，不生成或传播违法、违规及违背公序良俗的内容。

* **🌍 践行科技向善与社会责任：** AstroYingClaw 的初衷是降低天文教育的门槛。通过一句话生成课件与自动指星，我们希望让缺乏专业天文教师的偏远地区学校、普通兴趣班也能享受到沉浸式的星空课堂，用 AI 赋能科普普及，创造真实可见的社会教育价值。

* **©️ 尊重知识产权与科学严谨性：** 插件计算使用的星体数据源于公开合规的天文历表。对于 AI 自动生成的天文讲解词，我们致力于引导模型遵循客观科学事实。AstroYingClaw 项目本身基于 MIT 协议开源，尊重所有前置依赖库及硬件协作者（东北林业大学天文爱好者协会团队、硬件开发团队、云端大模型团队等）的知识产权与贡献。
